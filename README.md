...
We are utilizing the Flask, a Python framework for developing web applications to host our app locally. The buttons are connected to different routes which render different html docs. Code can be seen in routes.py and an the routing behind our homepage can be seen here:
```
@app.route("/")
def homepage():
...
1. Compare the job title that their resume most likely represents to the job title a selected job description most likely represents.
...
Right click on the link in the terminal to open application on the browser
